...
func (s HandlePost) CreateHandlerPost(ctx context.Context) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			utils.ErrorResponse(w, http.StatusBadRequest, "Error to create Post")
			fmt.Printf("Error to create Post: %v", err)
			return
		}

		utils.JSONResponse(w, http.StatusAccepted, created)
	}
}
...
func (s HandlePost) ListHandlerPost(ctx context.Context) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			fmt.Printf("Error to find Post: %v", err)
			return
		}
